...
<div class="mx-auto max-w-3xl space-y-6">
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <h2 class="text-2xl font-semibold text-primary">Editar Pedido</h2>
        <p class="text-sm text-slate-500">Actualiza la información del pedido seleccionado.</p>

        <form action="{{ route('orders.update', $order) }}" method="POST" class="mt-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="total" value="{{ old('total', $order->total) }}" class="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                    @error('total')
                        <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                    @enderror
                </div>
